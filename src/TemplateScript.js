...
/* menu links */
// In the function below, add more lines like "regexTool('link text','functionName()')" to add
...
window.rmflinks = function () {
	...
	regexTool('Formatação geral','generalFixes()');
	regexTool('Corrige assinatura','fixSignature()');
	regexTool('Corrige links HTTP','fixHTTPLinks()');
	switch( mw.config.get( 'wgDBname' ) ) {
	case 'ptwiki':
		regexTool('Usar "Ver também,..."','fixObsoleteTemplates()');
		regexTool('Corrigir fórmulas','fixMath()');
		regexTool('Corrigir listas','fixLists()');
		break;
	case 'ptwikisource':
		regexTool('Corrigir OCR', 'fixOCR()');
		break;
	case 'ptwikibooks':
		regexTool('Corrigi fórmulas','fixMath()');
		regexTool('Remover linhas duplicadas','dedupeList(editbox.value)');
		regexTool('Formatar cabeçalhos','formatHeadings()');
		regexTool('Formatar predefinições','formatTemplates()');
		regexTool('Formatar categorias','formatCategories()');
		regexTool('Corrigir listas','fixLists()');
		regexTool('Usar links relativos','abs2rel()');
		regexTool('Formatar links','formatLinks()');
		...
		regexTool('Gravar lista de capítulos (CUIDADO!)','saveChaptersList()');
		regexTool('TEST: Criar AutoNav','createAutoNav()');
		regexTool('TEST: Refs do Google Books','convertRefs()');
		regexTool('TEST: Wiki -> LaTeX','wiki2latex()');
		regexTool('TEST: LaTeX -> Wiki','latex2wiki()');
		break;
	default:
	...
	regexTool('Regex no sumário','usingRegex()');
...
// [[meta:User:Pathoschild/Script:Regex menu framework]]
...
window.dedupeList = function (lista){
...
window.geraPredef = function (){
	var	lista = window.dedupeList( geraLista() ),
...
window.geraCol = function (){
	var	lista = window.dedupeList( geraLista() ), i,
		col = '{'+'{Livro gravado\n |título={'
			+'{subst:SUBPAGENAME}}\n |subtítulo=\n |imagem da capa=\n'
			+' |cor da capa=\n}}\n\n== ' + nomeLivro + ' ==\n';
...
window.saveChaptersList = function (){
...
window.formatHeadings = function (){
	var oldText = editbox.value;
...
window.formatCategories = function (){
	var oldText = editbox.value;
...
window.formatLinks = function (){
	...
	regex(/\[(?:https?)?:\/\/pt.wikibooks.org\/w\/index.php\?title=Wikibooks/ig, '[//pt.wikibooks.org/w/index.php?title=Wikilivros');
	regex(/\[\[(?:Wikipedia|Project)( Discussão)?:/ig, '[[Wikipédia$1:');
	regex(/\[\[(?:Wikipedia|Project) Talk:/ig, '[[Wikipédia Discussão:');
	regex(/\[(?:https?)?:\/\/pt.wikipedia.org\/w\/index.php\?title=Wikipedia/ig, '[//pt.wikipedia.org/w/index.php?title=Wikipédia');
...
window.generalFixes = function (){
	...
	case 'ptwiki':
		...
		window.fixHTTPLinks();
		window.fixLists();
		window.fixObsoleteTemplates();
		window.formatLinks();
		window.fixImageLinks();
		break;
	case 'ptwikisource':
		break;
	case 'ptwikibooks':
...
window.removeOCRModernization = function (res){
	...
	// lc.conv_text_from_dic() está em [[oldwikisource:User:Helder.wiki/Scripts/LanguageConverter.js]]
...
window.fixOCR = function (){
	...
	if (mw.config.get('wgNamespaceNumber') === 106 ) {
		...
		url = mw.config.get('wgServer') + mw.config.get('wgScriptPath') + '/api.php?action=query&format=json&prop=revisions&titles=' + dictionaries + '&rvprop=content&callback=removeOCRModernization';
		mw.loader.load(url);
	}
	...
	$.each( tabela, function(id, palavra){
		...
		regex1 = new RegExp('\\b' + ucFirst(palavra) + '\\b', 'g');
		regex(regex1, ucFirst(palavra), 5);
	});
	/*jslint unparam: false*/
	...
	$.each( tabela, function(find, rep){
		...
		regex(re, rep, 5);
		//Converte também a palavra correspondente com a primeira letra maiúscula
		re = new RegExp('\\b' + ucFirst(find), 'g');
		regex(re, ucFirst(rep), 5);
	});
